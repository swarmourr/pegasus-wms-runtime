...
# Fill colors for each prediction status
PRED_STATUS_COLORS = {
    "NORMAL":     "#a8d5a2",   # green
    "SPARSE":     "#ffe28a",   # yellow
    "ZERO_SHOT":  "#ffb347",   # orange
    "RULE_BASED": "#aec6e8",   # blue
}
...
class Job(Node):
    def __init__(self):
        Node.__init__(self)
        self.xform = None
        # Runtime prediction fields (populated when --predictions is passed)
        self.pred_runtime_s = None
        self.pred_status    = None
        self.pred_low_s     = None
        self.pred_high_s    = None
        self.dag_level      = None
    ...
    def renderNode(self, renderer):
        ...
        # Annotate with prediction info when available
        if self.pred_runtime_s is not None:
            mins = max(1, int(self.pred_high_s // 60)) if self.pred_high_s else "?"
            label = (
                f"{label}\\n"
                f"~ {self.pred_runtime_s}s  [{self.pred_low_s}s – {self.pred_high_s}s]\\n"
                f"{self.pred_status}  |  max {mins} min"
            )
            if self.dag_level is not None:
                label = f"L{self.dag_level}  {label}"
            color = PRED_STATUS_COLORS.get(self.pred_status, renderer.getcolor(self.xform))
        else:
...
dot representation is output.""",
    ...
    parser.add_option(
        ...
        metavar="FILE",
        default=None,
        help="Path to runtime_predictions.json produced by PegasusOracle. "
             "When provided, each job node is annotated with predicted runtime, "
             "prediction status (NORMAL/SPARSE/ZERO_SHOT/RULE_BASED), DAG level, "
             "and confidence interval. Nodes are colored by status. [default: none]",
    )
    ...
    # Annotate job nodes with runtime predictions if provided
    if options.predictions:
        if not os.path.isfile(options.predictions):
            parser.error(f"Predictions file not found: {options.predictions}")
        with open(options.predictions) as pf:
            pred_data = json.load(pf)
        pred_map = {p["job_id"]: p for p in pred_data.get("predictions", [])}
        for node in dag.nodes.values():
            if isinstance(node, Job) and node.id in pred_map:
                p = pred_map[node.id]
                node.pred_runtime_s = p.get("predicted_runtime_s")
                node.pred_status    = p.get("status")
                node.pred_low_s     = p.get("lower_bound_s")
                node.pred_high_s    = p.get("upper_bound_s")
                node.dag_level      = p.get("dag_level")
