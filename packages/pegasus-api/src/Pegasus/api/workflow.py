...
class Workflow(Writable, HookMixin, ProfileMixin, MetadataMixin):
    ...
    def predict_runtimes(
        ...
        """
        Predict runtimes for ALL jobs in the workflow using the embedded V19
        PegasusOracle model — no external API server required.

        Runs a single batch inference pass over every job (source, middle, and
        sink jobs) and writes two output files to the output directory:

        - ``runtime_predictions.csv``  — one row per job
        - ``runtime_predictions.json`` — same data plus summary (status counts,
          condor slot info used, interval percentage, active config)

        **Three calling styles:**

        .. code-block:: python

            # 1 — simple: just the model path
            wf.predict_runtimes("pegasus_oracle_model.pkl")

            # 2 — with explicit output directory
            wf.predict_runtimes("pegasus_oracle_model.pkl", output_dir="outputs/")

            # 3 — full config object
            from Pegasus.runtime_predictor import RuntimePredictionConfig
            cfg = RuntimePredictionConfig(
                model_path       = "pegasus_oracle_model.pkl",
                output_dir       = "outputs/",
                q_lo             = 0.10,   # tighter interval
                q_hi             = 0.90,
                sparse_threshold = 20,
                slot_cpu_count   = 8,
                slot_ram         = 16_777_216.0,  # 16 GB in kB
            )
            wf.predict_runtimes(config=cfg)

            # 4 — from pegasus.properties
            from Pegasus.runtime_predictor import RuntimePredictionConfig
            cfg = RuntimePredictionConfig.from_properties(props)
            wf.predict_runtimes(config=cfg)

        :param model_path: path to the trained ``.pkl`` model file; ignored
            when *config* is provided
        :type model_path: Optional[Union[str, Path]]
        :param output_dir: directory for output files; ignored when *config* is
            provided; defaults to ``self._submit_dir`` or current directory
        :type output_dir: Optional[Union[str, Path]]
        :param config: full :py:class:`~Pegasus.runtime_predictor.RuntimePredictionConfig`
            object; when given, *model_path* and *output_dir* are ignored
        :type config: Optional[RuntimePredictionConfig]
        :raises ImportError: if PyTorch or scikit-learn are not installed
        :raises FileNotFoundError: if the model file does not exist
        :return: list of prediction dicts (one per job)
        :rtype: List[dict]
        """
        try:
            ...
            raise ImportError(
                "Runtime prediction requires torch and scikit-learn. "
        ...
        if config is None:
            resolved_out = (
                ...
                else getattr(self, "_submit_dir", None) or str(Path.cwd())
            )
        ...
        return WorkflowRuntimePredictor(config).predict(self)
